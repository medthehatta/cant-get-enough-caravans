...
func _move():
    ...
    current_tile = route[tile_index]
    caravan_scn.position = map.map_tile_to_control(current_tile.map_position)
    print(
        "idx={} map_pos={} pos={}".format(
            [tile_index, current_tile.map_position, caravan_scn.position],
            "{}",
        )
    )

    if remaining_tile_progress == null:
        remaining_tile_progress = _progress_required_for(current_tile)
...
func traverse(delta: float) -> bool:
    ...
    var modified_progression = _base_progress_for(current_tile).modify_with([caravan])
    remaining_tile_progress -= modified_progression.speed * delta

    # Inflict integrity damage
    var modified_damage = _base_damage_for(current_tile).modify_with([caravan])
    caravan.inflict_damage_to_caravan(modified_damage)

    # Inflict stress
    var modified_stress = _base_stress_for(current_tile).modify_with([caravan])
    caravan.inflict_stress_to_caravan(modified_stress)

    # Contribute XP
    var modified_xp = _base_xp_for(current_tile).modify_with([caravan])
    caravan.contribute_xp_to_caravan(modified_xp)

    # Consume resources
    var modified_consumption = _base_consumption_for(current_tile).modify_with([caravan])
    caravan.consume_caravan_resources(modified_consumption)

    # Drop loot
    var modified_loot = _base_loot_for(current_tile).modify_with([caravan])
